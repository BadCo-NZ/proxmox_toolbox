  ...
  do
    if [[ $opt = "" ]]; then
      ...
      case $opt in
	   ...
	   ;;
      3) clear;
			read -p "- This will install thoses libraries if missing: ifupdown2 - git - sudo - libsasl2-modules - lshw - lm_sensors - Continue? y = yes / anything = no: " -n 1 -r
			if [[ $REPLY =~ ^[Yy]$ ]]; then
				echo " "
				...
				if [ $(dpkg-query -W -f='${Status}' lm_sensors 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
					apt-get install -y lm_sensors;.
				else
					echo "- lm_sensors already installed"
				fi
			sleep 3
			fi	
		main_menu
